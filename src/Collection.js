...
const Collection = () => {
    ...
    const [items, setItems] = useState(allCollectionItems);
    const [searchVal, setSearchVal] = useState("");

    function handleSearchClick() {
        //if search is empty return all items
        if (searchVal === "") {
            setItems(allCollectionItems);
            return;
        }

        //if search value entered, use to filter results
        const filterBySearch = allCollectionItems.filter((item) => {
    ...
    return (
        <div>
            ...
            <div class="Collection-List">
                ...
                />
                <button class="search-button" onClick={handleSearchClick}>Search</button>
